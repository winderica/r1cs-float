...
use crate::r1cs_std::{fields::fp::FpVar, prelude::*, Assignment, ToConstraintFieldGadget, Vec};
...
impl<F: Field> Boolean<F> {
    ...
    ///
    /// ```
    /// # fn main() -> Result<(), crate::r1cs::SynthesisError> {
    /// // We'll use the BLS12-381 scalar field for our constraints.
    /// use ark_test_curves::bls12_381::Fr;
    /// use crate::r1cs::*;
    /// use crate::r1cs_std::prelude::*;
    ///
        ...
        F: PrimeField,
    {
        ...
        if bits.is_constant() {
            ...
            Ok(crate::r1cs_std::fields::fp::AllocatedFp::new(value, variable, cs.clone()).into())
        }
    }
...
#[cfg(test)]
mod test {
    use crate::r1cs::{ConstraintSystem, Namespace, SynthesisError};
            ...
            for b_bool in [false, true].iter().cloned() {
                for a_neg in [false, true].iter().cloned() {
                    for b_neg in [false, true].iter().cloned() {
                        ...
                        let false_cond =
                            Boolean::new_witness(crate::ns!(cs, "cond"), || Ok(false))?;
    ...
    fn test_boolean_xor() -> Result<(), SynthesisError> {
        for first_operand in VARIANTS.iter().cloned() {
            for second_operand in VARIANTS.iter().cloned() {
                ...
                let a = construct(crate::ns!(cs, "a"), first_operand)?;
                let b = construct(crate::ns!(cs, "b"), second_operand)?;
        ...
        for condition in VARIANTS.iter().cloned() {
            for first_operand in VARIANTS.iter().cloned() {
                for second_operand in VARIANTS.iter().cloned() {
                    ...
                    let cond = construct(crate::ns!(cs, "cond"), condition)?;
                    let a = construct(crate::ns!(cs, "a"), first_operand)?;
                    let b = construct(crate::ns!(cs, "b"), second_operand)?;
