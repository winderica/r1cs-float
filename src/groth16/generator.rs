use super::logderivarg::generate_commitment;
...
use ark_std::{cfg_into_iter, cfg_iter, cfg_iter_mut, end_timer, start_timer};
...
impl<E: Pairing, QAP: R1CSToQAP> Groth16<E, QAP> {
        ...
        C: ConstraintSynthesizer<E::ScalarField>,
    {
        ...
        generate_commitment(cs.clone(), |_| None)?;
        end_timer!(synthesis_time, || format!("({})", cs.num_constraints()));
        ...
        let l =
            cfg_iter!(a[num_instance_variables..num_instance_variables + num_witness_variables])
                .zip(&b[num_instance_variables..num_instance_variables + num_witness_variables])
                .zip(&c[num_instance_variables..num_instance_variables + num_witness_variables])
                .map(|((a, b), c)| (beta * a + &(alpha * b) + c) * &delta_inverse)
                .collect::<Vec<_>>();

        let mut k = cfg_iter!(a[num_instance_variables + num_witness_variables..])
            .zip(&b[num_instance_variables + num_witness_variables..])
            .zip(&c[num_instance_variables + num_witness_variables..])
            .map(|((a, b), c)| (beta * a + &(alpha * b) + c) * &gamma_inverse)
            .collect::<Vec<_>>();
        ...
        let k_query = FixedBase::msm::<E::G1>(scalar_bits, g1_window, &g1_table, &k);
        end_timer!(k_time);

        let pedersen_g = E::G2Affine::rand(rng);
        let sigma = E::ScalarField::rand(rng);
        let pedersen_g_inv_neg_sigma = pedersen_g * (-sigma.inverse().unwrap());

        // Compute the K-query^sigma
        let k_sigma_time = start_timer!(|| "Calculate K^sigma");
        cfg_iter_mut!(k).for_each(|v| *v *= sigma);
        let k_query_sigma = FixedBase::msm::<E::G1>(scalar_bits, g1_window, &g1_table, &k);
        drop(k);
        end_timer!(k_sigma_time);
        ...
        let vk = VerifyingKey::<E> {
            ...
            pedersen_g,
            pedersen_g_inv_neg_sigma: pedersen_g_inv_neg_sigma.into_affine(),
        };
        ...
        let k_query_sigma = E::G1::normalize_batch(&k_query_sigma);
